...
- (id) initWithFrame:(CGRect)theFrame cancelEnabled:(BOOL)isCancelEnabled oneDMode:(BOOL)isOneDModeEnabled showLicense:(BOOL)showLicenseButton {
  ...
  if( self ) {
      ...
      if([UIScreen mainScreen].bounds.size.height>480)
      {
          ...
          if (!oneDMode) {
              cropRect = CGRectMake(kPadding, (self.frame.size.height - rectSize) / 2-89, rectSize, rectSize);
          } else {
              CGFloat rectSize2 = self.frame.size.height - kPadding * 2;
              cropRect = CGRectMake(kPadding, kPadding-89, rectSize, rectSize2);
          }
      }
      else
      {
            CGFloat rectSize = self.frame.size.width - kPadding * 2;
            if (!oneDMode) {
              cropRect = CGRectMake(kPadding, (self.frame.size.height - rectSize) / 2-45, rectSize, rectSize);
            } else {
              CGFloat rectSize2 = self.frame.size.height - kPadding * 2;
              cropRect = CGRectMake(kPadding, kPadding-45, rectSize, rectSize2);
            }
      }

      if([UIScreen mainScreen].bounds.size.height<=480)      {
           self.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"camerBg_4.png"]];
      }
      else
      {
          self.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"camerBg.png"]];
      }
//      self.backgroundColor = [UIColor clearColor];
    ...
    if (self.cancelEnabled) {
      self.cancelButton = [[UIButton alloc] initWithFrame:CGRectMake((self.frame.size.width-178)/2, cropRect.origin.y+cropRect.size.height+85, 178, 38)];
      if ([self.cancelButtonTitle length] > 0 ) {
        [cancelButton setTitle:self.cancelButtonTitle forState:UIControlStateNormal];
      } else {
        [cancelButton setTitle:@"取消扫描" forState:UIControlStateNormal];
      }
...
- (void)scanAnimation
{
    //UIImage *image = [self createImageWithColor:[UIColor blueColor] rect:CGRectMake(cropRect.origin.x, cropRect.origin.y, cropRect.size.width, 1)];
    if(!scanImgview)
    {
        scanImgview = [[UIImageView alloc] initWithFrame:CGRectMake(160, cropRect.origin.y+5, cropRect.size.width, 1)];
        //[imgview setImage:image];
        [scanImgview setBackgroundColor:[UIColor redColor]];
        [self addSubview:scanImgview];
        
        [scanImgview release];
    }
    CGPoint startPoint = CGPointMake(160, cropRect.origin.y);
    CGPoint endPoint = CGPointMake(160, cropRect.origin.y+cropRect.size.height);
    CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"position"];
    animation.fromValue = [NSValue valueWithCGPoint:startPoint];
    animation.toValue = [NSValue valueWithCGPoint:endPoint];
    animation.duration = 3.0f;
    animation.repeatCount = HUGE_VALF;
    [scanImgview.layer setPosition:startPoint];
    [scanImgview.layer addAnimation:animation forKey:nil];
}

- (UIImage *) createImageWithColor: (UIColor *) color rect:(CGRect)aRect
{
    CGRect rect= aRect;//CGRectMake(0.0f, 0.0f, 1.0f, 1.0f);
...
- (void)drawRect:(CGRect)rect {
  ...
  if (displayedMessage == nil) {
    self.displayedMessage = @"";// @"将条形码和液晶屏放在框内,自动扫描";//NSLocalizedStringWithDefaultValue(@"OverlayView displayed message", nil, [NSBundle mainBundle], @"Place a barcode inside the viewfinder rectangle to scan it.", @"Place a barcode inside the viewfinder rectangle to scan it.");
  }
  ...
  //	CGContextSetStrokeColor(c, white);
	...
	if (oneDMode) {
        NSString *text = @"";//@"将条形码和液晶屏放在框内,自动扫描";//NSLocalizedStringWithDefaultValue(@"OverlayView 1d instructions", nil, [NSBundle mainBundle], @"Place a red line over the bar code to be scanned.", @"Place a red line over the bar code to be scanned.");
